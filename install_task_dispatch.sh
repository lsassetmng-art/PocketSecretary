...
# ---- guard ----
[ -d "$FOUNDATION" ] || { echo "❌ erp-foundation not found"; exit 1; }
[ -f "$PM_LOOP" ] || { echo "❌ pm_loop.sh not found"; exit 1; }

# ---- backup (non-destructive) ----
[ -f "$PM_LOOP.bak_dispatch" ] || cp "$PM_LOOP" "$PM_LOOP.bak_dispatch"

# ---- rewrite pm_loop.sh with dispatch ----
cat <<'EOT' > "$PM_LOOP"
#!/bin/sh
set -eu
...
echo "✅ pm_loop updated with task dispatch"
exit 0
